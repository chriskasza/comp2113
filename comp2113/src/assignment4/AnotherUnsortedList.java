 ...
 * Assignment #: 04
 * Version:      1.0
 * File:         AnotherUnsortedList.java
 *
 * Another class for a generic unsorted list.  Order is preserved.
 */
package Assignment4;

import ch04.genericLists.*;

public class AnotherUnsortedList extends NewList {
	 ...
	 */
	public AnotherUnsortedList(int size) {
		super(size);
		if(size < 0) {
	...
	public void delete(Listable item) {
		if(isThere(item)) {
			for(int i = foundIndex; i < numItems; i++) {
				list[i] = list[i+1];
			}
			numItems--;
			foundIndex = -1;
